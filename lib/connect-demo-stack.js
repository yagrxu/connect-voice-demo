  ...
  constructor(scope, id, props) {
    ...
    if (enableAiAgent) {
      ...
      const promptYaml = [
        ...
        '  Always reply in the same language the caller used — Chinese in, Chinese out;',
        '  English in, English out.',
        '  Callers may mix English words into Chinese speech (for example city names);',
        ...
        '  Always use a tool to get the time or weather; never guess.',
    ...
    // When a custom domain is set, a viewer-request CloudFront Function rejects
    // any request whose Host header is not the custom domain — so the raw
    // *.cloudfront.net URL returns 403 and access is forced through the domain.
    let fnAssoc;
    if (domainName) {
      const hostFn = new cloudfront.Function(this, `${PREFIX}HostGuard`, {
        code: cloudfront.FunctionCode.fromInline(
          "function handler(event){var h=event.request.headers.host&&event.request.headers.host.value;" +
          "if(h!=='" + domainName + "'){return{statusCode:403,statusDescription:'Forbidden'," +
          "body:{encoding:'text',data:'Access only via " + domainName + "'}};}return event.request;}"
        ),
        comment: 'Force access through the custom domain (403 for other Host headers).',
      });
      fnAssoc = [{ function: hostFn, eventType: cloudfront.FunctionEventType.VIEWER_REQUEST }];
    }
    ...
    const distribution = new cloudfront.Distribution(this, `${PREFIX}Cdn`, {
      ...
      additionalBehaviors: {
        // Both API routes go to API Gateway (not S3). /transcript needs the
        // query string (since=...) forwarded, which ALL_VIEWER_EXCEPT_HOST_HEADER does.
        'webcall': apiBehavior(),
